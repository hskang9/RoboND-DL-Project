...
In this project, fully convolutional network(FCN) is trained to identify a target and track it in simulation. “follow me” applications which is used in this project can be used to many fields of robotics, and this technique can be extended to scenarios such as advanced cruise control in autonomous vehicles or human-robot collaboration in industry.

[image_0]: ./docs/misc/sim_screenshot.png
![alt text][image_0] 

### Setup instructions
[README](README.md)  provides instructions for installing and training the model.
### Network architecture

The suggested network in the project is a Fully Convolutional Network(which means there is no fully connected neural network). The network follows an Encoder/Decoder based architecture to 
support pixel level classification instead of bbox regression(YOLO, SSD).   

Here are the layers to build the network.

#### 1 X 1 Convolutions
![alt](./docs/misc/fcn_1.png)    

1 X 1 Convolutions are essentially a very inexpensive way of making deep learning models deeper and have more parameters without completely changing their structure. since it convolves based on just one pixel each. Also, they are mathmatically cheap to calculate, because it is just multiplying matrices together instead of convolving it.

1 X 1 Convolution in this project will be used for classifying objects eventually giving semantics to segmentation network. You can think of it as "Dense layer with spatial information". 

Dense layer was not used because it changes dimension, activating upon numerous features as well as spatial information.
